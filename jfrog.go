...
type JFrogResponse struct {
  Data []JFrogArtifact `json:"results"`
}
...
type JFrogPayload struct {
  Checksum string `json:"checksum"`
  Search   string `json:"search"`
}

func getPayload(sha1 string) JFrogPayload {
  return JFrogPayload{
    Checksum: sha1,
    Search:   "checksum",
  }
...
func jfrog(sha1 string) (dep Dependency, err error) {
  var (
    content      []byte
    req          *http.Request
    res          *http.Response
    repoResponse JFrogResponse
  )
  reqUrl := "https://oss.jfrog.org/ui/artifactsearch/checksum"
  ...
  //debugf("JFrog Response %v", repoResponse)
